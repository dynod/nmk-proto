...
Dynamically lists all sub-folders of {ref}`${protoFolder}<protoFolder>` (exactly one per proto file, in same order than {ref}`${protoInputFiles}<protoInputFiles>`).
...
Dynamically lists all sub-folders of {ref}`${protoFolder}<protoFolder>` (no duplicates).
...
If set to true, disable all generated code check tasks (see {ref}`proto.check.py<proto.check.py>` task).
...
_<span style="color:orange">Changed in version 1.2.0</span>_ -- Previous wildcard was a simple `*`
